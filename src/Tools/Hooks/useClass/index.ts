...
const useClass = <T extends OBJ>(CLASS_NAME: SUG<CLASS_NAMES>, options?: UC_Options) => {
	...
	const { [CLASS_NAME]: LocalData }: any = useLocalStorage();
	...
	const putItem = async (item: T, putItemOption: UC_putItem_Options = {}) => {
		...
		return result as OBJ & { item?: T; message?: string; action?: 'add' | 'edit' };
	};
	...
	const addItem = async (item: T) => {
		...
		return result as OBJ & { item?: T; message?: string };
	};
	...
	const deleteItem = async (id: string, body?: OBJ<any>) => {
		_emitLoading('deleteItem', true);
		let result;
		try {
			body = { item: { [PRIMARY_KEY]: id }, ...(body || {}) };
			result = await Delete({ url: URL, body });
			dispatch(setLocalData({ class_name: CLASS_NAME, partialUpdate: true, data: [], deleted: [id] }));
		} catch (e) {
			throw e;
		} finally {
			_emitLoading('deleteItem', false);
		}
		return result as OBJ & { item?: T; message?: string; action?: 'delete' };
	};
	...
	const Class = {
		...
		items: LocalData as T[] | undefined,

		item: useCallback(
			(ID?: string) => {
				if (!ID) return;
				return (LocalData as any[])?.find(item => item[PRIMARY_KEY] === ID) as T | undefined;
			},
			[LocalData]
		),
	};
